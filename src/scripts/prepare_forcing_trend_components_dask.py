...
def detrend_forcing_dask(forcing_file_path, output_path, method='vectorized',
    ...
    # Prepare encoding for chunked output. Chunk time into a single write-friendly chunk
    # to avoid many small writes which are slow on shared filesystems.
    ...
    logger.info(f'Writing trend dataset to {output_path} with chunks {chunksizes} (no_compress={no_compress})')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-compress', action='store_true', help='Disable zlib compression for output')
    ...
    detrend_forcing_dask(args.input_file, args.output_file, method=args.method,
                         ...
                         compression=args.compression, no_compress=args.no_compress, use_dask=args.use_dask,
