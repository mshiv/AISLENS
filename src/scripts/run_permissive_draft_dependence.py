...
PARAMETER_SETS = {
    ...
    'moderate': {
        'min_r2_threshold': 0.05,  # Lower R2 requirement
    ...
    'permissive': {
        'min_r2_threshold': 0.03,  # Much lower R2
    ...
    'very_permissive': {
        'min_r2_threshold': 0.02,  # Very low R2
    ...
    'ultra_permissive': {
        'min_r2_threshold': 0.01,  # Minimal R2 requirement
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog='Examples:\n'
               ...
               '  python run_permissive_draft_dependence.py --parameter_set moderate --verbose'
    )
    parser.add_argument('--parameter_set', choices=list(PARAMETER_SETS.keys()), default='permissive',
                       help='Parameter set to use (default: permissive)')
    parser.add_argument('--list_sets', action='store_true', help='List available parameter sets and exit')
    ...
    if args.list_sets:
        ...
        for name, params in PARAMETER_SETS.items():
            ...
            logger.info(f"  R2: {params['min_r2_threshold']}, Corr: {params['min_correlation']}, Ruptures: {params['ruptures_penalty']}, Bins: {params['n_bins']}, Min pts/bin: {params['min_points_per_bin']}")
        return
    
    # Setup logging
    log_file = setup_logging(config.DIR_ICESHELF_DEDRAFT_SATOBS, verbose=args.verbose)
    
    logger.info("=" * 80 + "\nPERMISSIVE DRAFT DEPENDENCE ANALYSIS\n" + "=" * 80)
    
    # Get parameter set and log details
    ...
    all_results, all_draft_params = calculate_draft_dependence_comprehensive(
        icems, satobs, config, **run_params, noisy_fallback='zero', model_selection='best'
    )
    ...
    if all_draft_params:
        ...
        linear_percentage = (linear_count / total_count * 100) if total_count else 0
    ...
    # Suggest next steps
    logger.info("\n" + "=" * 80 + "\nNEXT STEPS\n" + "=" * 80)
    logger.info(f"1. Visualize results:\n   python visualize_draft_dependence.py --parameter_set {args.parameter_set}")
    logger.info(f"2. Compare with other parameter sets:\n   python visualize_draft_dependence.py --create_summary")
