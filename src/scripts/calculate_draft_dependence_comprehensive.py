...
def calculate_draft_dependence_comprehensive(icems, satobs, config,
    ...
    for i, shelf_name in enumerate(shelf_names):
        ...
        try:
            ...
            # DEBUG: Check ice shelf geometry before processing
            ice_shelf_geom = icems.iloc[actual_index].geometry
            print(f"  Ice shelf bounds: {ice_shelf_geom.bounds}")
...
def merge_comprehensive_parameters(all_draft_params, icems, satobs, config, save_dir):
    ...
    # Use alphabetical order for simple merging
            ...
            if param_file.exists():
                try:
                    ...
                    if len(merged_dataset.data_vars) == 0:
                        ...
                        # Subsequent files - handle paramType conflicts specially
                        if config_param_name == 'draftDepenBasalMelt_paramType':
                            # For paramType, use override to handle conflicting values
                            merged_dataset = xr.merge([merged_dataset, shelf_ds], compat='override')
                        else:
                            # For other parameters, use default merge
