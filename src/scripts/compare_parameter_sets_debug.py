...
def load_summary_data(parameter_sets: List[str], base_dir: Path) -> Dict[str, pd.DataFrame]:
    """Load summary data for each parameter set."""
    summaries = {}
...
def load_parameter_grids(parameter_sets: List[str], base_dir: Path) -> Dict[str, xr.Dataset]:
    """Load parameter grids for each parameter set."""
    param_grids = {}
    for param_set in parameter_sets:
...
def create_comparison_table(summaries: Dict[str, pd.DataFrame], output_dir: Path):
    """Create a comparison table of parameter set statistics."""
    comparison_data = []
...
def extract_scalar(arr):
    """Extract a scalar value from an array, handling various array shapes."""
    arr = np.asarray(arr)
    if arr.size == 0:
...
def plot_shelf_data(ax, plot_melt, plot_draft, pred_melt_vis, is_linear, min_draft):
    """Plot observed and predicted data on the given axis."""
...
def format_shelf_axis(ax, plot_melt, plot_draft):
    """Format axis limits and styling for shelf comparison plots."""
...
def create_shelf_comparison_plot(shelf_name: str, shelf_idx: int,
                                 ...
                                 output_dir: Path,
                                 base_dir: Path):
    """Create comparison plot for a single ice shelf across parameter sets."""
    ice_shelf_geom = icems.iloc[shelf_idx].geometry
    if ice_shelf_geom is None or ice_shelf_geom.is_empty:
        logger.warning(f"Empty geometry for shelf '{shelf_name}'")
        return None
    ...
    if valid_mask.sum() == 0:
        logger.warning(f"No valid data for shelf '{shelf_name}'")
        return None
    ...
    for idx, param_set in enumerate(parameter_sets):
        ...
        # Subsample for plotting (max 2000 points for performance)
        max_plot_points = 2000
        n_plot = min(max_plot_points, len(obs_draft))
    ...
    # observed data will be drawn inside plot_shelf_data (so don't draw it twice)
        ...
        if param_set in summaries:
            ...
            if not shelf_param_file:
                logger.warning(f"Parameter file not found for shelf '{shelf_name}' in parameter set '{param_set}'")
                ...
                try:
                    ...
                    logger.debug(f"[{shelf_name} | {param_set}] plot_draft range: {plot_draft.min()} to {plot_draft.max()} | "
                               f"plot_melt range: {plot_melt.min()} to {plot_melt.max()} | "
                               f"pred_melt_vis range: {np.nanmin(pred_melt_vis)} to {np.nanmax(pred_melt_vis)} | "
                               f"valid_mask_pred sum: {np.sum(valid_mask_pred)}")
                    
                    if np.sum(valid_mask_pred) > 0:
                        melt_valid = plot_melt[valid_mask_pred]
                        pred_valid = pred_melt_vis[valid_mask_pred]
                        
                        logger.debug(f"[{shelf_name} | {param_set}] melt_valid sample: {melt_valid[:5]} | "
                                   f"pred_valid sample: {pred_valid[:5]}")
                        
                        mse = np.mean((melt_valid - pred_valid)**2)
                        
                        # Calculate R² with proper handling of edge cases
                        ss_res = np.sum((melt_valid - pred_valid)**2)
                        ss_tot = np.sum((melt_valid - np.mean(melt_valid))**2)
                        ...
                        logger.warning(f"[{shelf_name} | {param_set}] No valid predictions")
                    ...
                    logger.error(f"Error processing parameters for {shelf_name} | {param_set}: {e}")
                    ax.set_title(f"{param_set}\nError loading params", fontsize=9, pad=10)
            else:
                ax.set_title(f"{param_set}\nNo predictions", fontsize=9, pad=10)
        else:
            ax.set_title(f"{param_set}", fontsize=9, pad=10)
        
        # Set axis labels with correct units (m/yr after conversion)
        ax.set_xlabel('Melt Rate (m yr⁻¹)', fontsize=8)
...
def create_metric_comparison_plots(summaries: Dict[str, pd.DataFrame], output_dir: Path):
    """Create plots comparing metrics across parameter sets."""
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    param_sets = list(summaries.keys())
    
    meaningful_counts = [summaries[ps]['is_meaningful'].sum() for ps in param_sets]
    linear_counts = [summaries[ps]['is_linear'].sum() if 'is_linear' in summaries[ps].columns else 0 
                    for ps in param_sets]
    constant_counts = [summaries[ps]['is_constant'].sum() if 'is_constant' in summaries[ps].columns else 0 
                      for ps in param_sets]
    
    # Bar plot: parameterization types
    ax = axes[0, 0]
    ...
    # Boxplot: Correlation distribution
    ax = axes[1, 0]
...
def find_best_parameter_set(summaries: Dict[str, pd.DataFrame], 
    ...
    """Find the best parameter set for each shelf based on specified criterion."""
    all_shelves = set()
...
def main():
    ...
    if param_grids:
        ...
        for shelf_name in shelves_to_plot:
            try:
                # Find shelf index with error handling
                shelf_matches = icems[icems['name'] == shelf_name]
                if len(shelf_matches) == 0:
                    logger.warning(f"Shelf '{shelf_name}' not found in ice shelf masks")
                    continue
                
                shelf_idx = shelf_matches.index[0]
                ...
                logger.error(f"  Failed to plot {shelf_name}: {e}", exc_info=True)
