...
def prepare_model_simulation():
    ...
    print("Draft dependence predictions merged and reindexed successfully.")
    ...
    #model_seasonality = xr.open_dataset(config.FILE_SEASONALITY)
    #model_variability = xr.open_dataset(config.FILE_VARIABILITY)
    ...
    model_variability_extrapl = model_variability_extrapl.fillna(0)  # Fill NaN values with 0 before saving
    model_variability_extrapl.to_netcdf(config.FILE_VARIABILITY_EXTRAPL)
    print("Model variability extrapolated successfully.")
    model_seasonality_extrapl = extrapolate_catchment_over_time(model_seasonality, 
                                                                icems, config, 
                                                                config.SORRM_FLUX_VAR
                                                                )
    model_seasonality_extrapl = model_seasonality_extrapl.fillna(0)  # Fill NaN values with 0 before saving
