...
for ii, run in enumerate(runs):
    ...
    for row, (variable, log, colormap, cbar_ax, vmin, vmax) in enumerate(
        ...
        if 'cellMask' in m.variables.keys():
            ...
            floatingIceMask = (initialExtentMask & ~groundingLineMask).astype(bool)
            initialFloatingMask = (cellMask[0] & floatValue) // floatValue
            ...
            calc_mask = True
            groundedMask = (m.variables['thickness'][:] > (-rhosw / rhoi * m.variables['bedTopography'][:]))
            groundingLineMask = groundedMask.copy()  # This isn't technically correct, but works for plotting
            initialExtentMask = (m.variables['thickness'][:] > 0.)
            floatingIceMask = (initialExtentMask & ~groundingLineMask).astype(bool)
            initialFloatingMask = (m.variables['thickness'][0] <= (-rhosw / rhoi * m.variables['bedTopography'][0]))
        else:
            print(f'cellMask and thickness and/or bedTopography not present in output file {run};'
                   ' Skipping mask calculation.')
            initialFloatingMask = np.zeros_like(var_to_plot[0])
            calc_mask = False
        ...
        for col, timeLev in enumerate(timeLevs):
            ...
            #masked_var = np.ma.masked_where(floatingIceMask[timeLev, :] == 1, var_to_plot[timeLev, :])
            masked_var = np.ma.masked_where(floatMask[timeLev, :] == 0, var_to_plot[timeLev, :])
            ...
            varPlot[run][variable].append(
                              axs[index].tripcolor(
                                 triang, masked_var, cmap=colormap,
                                 shading='flat', norm=norm))
            
            #floating_ice = axs[index].tripcolor(triang, floatMask[timeLev, :],
            #                                        cmap='cool')#, alpha=0.5, vmin=0, vmax=1)

            axs[index].set_aspect('equal')
            axs[index].set_title(f'year = {yr[timeLev]:0.2f}')

        vmin = np.nanmin(masked_var)
        vmax = np.nanmax(masked_var)
        norm = Normalize(vmin=vmin, vmax=vmax)
        ...
        #cbars.append(Colorbar(ax=cbar_ax, mappable=floating_ice, orientation='vertical',
        #         label='floating ice'))
