...
for i, time_step in enumerate(valid_time_steps):
    ...
    # Plot histogram
    #ax.hist(var_values, bins=min(10, len(var_values)//2), alpha=0.7, density=True, 
    #        color='skyblue', edgecolor='black', label='Data')

    # Plot histogram with better binning logic
    #n_bins = max(5, min(15, len(var_values)))  # Between 5 and 15 bins
    #ax.hist(var_values, bins=n_bins, alpha=0.7, density=True, 
    #        color='skyblue', edgecolor='black', label='Data')
    
    # Plot histogram with adaptive binning
    """
    if len(var_values) <= 5:
        n_bins = len(var_values)  # One bin per data point for very small samples
    elif len(var_values) <= 10:
        n_bins = max(5, len(var_values) // 2 + 2)  # 5-7 bins for small samples
    else:
        n_bins = min(15, int(np.sqrt(len(var_values))) + 3)  # Square root rule + buffer
...
# Determine appropriate units (same logic as before)
if 'volume' in options.variable.lower() or 'vaf' in options.variable.lower():
    if options.plotChange:
        unit_str = f' change ({massUnit})'
    elif options.plotPercentChange:
