...
echo "Creating symlinks for NetCDF files..."
...
for scenario in "${ISMIP6_SCENARIOS[@]}"; do
    ...
    if [ -d "$ISMIP6_PATH" ]; then
        ...
        for file in "${FILES[@]}"; do
            ...
            target_link="${DIAGNOSTICS_PATH}/ISMIP6/${scenario}/${file}"
...
echo "Done! Symlinks created in $DIAGNOSTICS_PATH"
...
find "$DIAGNOSTICS_PATH" -type l | sort
