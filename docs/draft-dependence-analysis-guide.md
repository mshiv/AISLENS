...
This document describes the draft dependence parameterization analysis developed for the AISLENS project. The analysis creates parameters that relate ice shelf basal melt rates to draft depth for use in the MPAS-Albany Land Ice sheet model.
...
The script generates 5 parameter fields required by the MALI ice sheet model for this workflow:

1. **`draftDepenBasalMelt_minDraft`**: Threshold draft depth (meters) - deeper than this gets a linear relationship
...
The ice sheet model applies the parameterization using a simple if/else choice:
...
Ice shelves are processed starting from index 33 (Abbott Ice Shelf) for the given `icems` file - this excludes multipolygon regions(Filchner-Ronne, Ross etc.), larger subcontinental regions (WAIS, EAIS, AP) and the IMBIE regions to focus only on individual ice shelves (100 in total spanning the continent in this study).
...
NaN values in the initialization file for MALI introduce other workflow changes to be implemented, so this has been skipped as of now.
...
This typically gives 55-70% linear dependence coverage with acceptable quality for our use case here of forcing the ice sheet model with variability. Also note that the intended purpose here is to run ensembles of realistic forcing variability, and not, by design, actual realistic projections of ice shelf melt.
